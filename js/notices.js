  ...
  function renderHistory() {
    ...
    try {
      ...
      el.innerHTML = hist.map(h =>
        ...
        + '<div style="font-size:var(--fs-14);color:var(--tm);font-family:monospace;">' + esc(h.doi) + '</div>'
        + '</div>'
        + '<div style="display:flex;gap:6px;flex-shrink:0;">'
        + '<button onclick="fillDoi(\'' + esc(h.doi) + '\')" style="font-size:var(--fs-14);padding:3px 8px;border:1px solid var(--border2);border-radius:4px;background:var(--surface);cursor:pointer;color:var(--accent);">다시 조회</button>'
        + '</div>'
        + '</div>'
      ).join('');
    } catch(e) {}
